...
import fun.efto.luna.core.plugin.builtin.invocation.model.*;
...
public class InvocationTraceProbe {
    ...
    // Defensive serialization limits
    ...
    private static void doMethodExit(String className, String methodName,
        ...
        try {
            ...
            if (isRoot) {
                ...
                ProbeOutput.offer(new ProbeMessage("INVOCATION", payload, structuredPayload));
        ...
        try {
            ...
            if (value instanceof Map) {
                ...
                for (Map.Entry<?, ?> entry : map.entrySet()) {
                    ...
                    if (count > 0) sb.append(", ");
                    sb.append(serializeValue(entry.getKey())).append("=").append(serializeValue(entry.getValue()));
                    count++;
                }
                sb.append("}");
                return sb.toString();
            }
            // Object type - use toString with truncation
