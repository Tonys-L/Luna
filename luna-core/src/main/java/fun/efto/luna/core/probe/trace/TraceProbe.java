package fun.efto.luna.core.probe.trace;
...
public class TraceProbe {
    ...
    public static void onTraceEnd(String className, String methodName, long thresholdMs) {
        ...
        if (elapsedMs >= thresholdMs) {
            ProbeOutput.offer(new ProbeMessage("TRACE", "[TRACE] " + className + "." + methodName + " took " + elapsedMs + " ms", null, "text"));
        }
    }
    ...
    public static void onTraceAlert(String className, String methodName, long thresholdMs) {
        ...
        long elapsedMs = (System.nanoTime() - start) / 1_000_000;
        if (elapsedMs >= thresholdMs) {
            ProbeOutput.offer(new ProbeMessage("TRACE", "[SLOW] " + className + "." + methodName + " took " + elapsedMs + " ms (threshold=" + thresholdMs + " ms)", null, "text"));
        }
    }
}
